...
export async function runSectionSuggestion(params: {
  ...
  let prompt = `Архитектурное решение: "${title}". Секция: "${sectionLabels[section]}".\n\n`;
  if (currentContent?.trim()) {
    prompt += `Существующий текст:\n${currentContent}\n\n`;
    prompt += `Напиши ТОЛЬКО новые дополнения, которых нет в существующем тексте. Не повторяй уже написанное. Не используй фразы "предлагаю", "рекомендую", "следует добавить". Пиши сразу текст дополнений.\n\n`;
  } else {
    prompt += `Напиши содержание для этой секции. Не используй фразы "предлагаю", "рекомендую". Пиши сразу текст.\n\n`;
  }
  prompt += `Формат: структурированный markdown. Для секции "варианты" используй:\n### Option A: Название\nОписание\n### Option B: Название\nОписание\n\n`;
  prompt += `Только текст дополнений, без вводных фраз. Русский язык. Максимум 200 слов.`;
